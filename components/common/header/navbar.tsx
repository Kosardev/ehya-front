...
console.log(">>>>>>>>>>>>>>>>>",menu)
    return (
       <div className={'group-hover:bg-white '}>
           <BaseContainer>
               <nav  className={'flex pt-2 px-2 lg:px-0 lg:pt-4 text-white justify-between lg:justify-start lg:gap-24 items-stretch'}>
                   <Icon onClick={()=>{setShowMobileMenu(true)}}
                         name="hamburger-menu" className='flex items-center justify-center lg:hidden'
                         color="fill-white group-hover:fill-red-ehya"/>
                   ...
                   <ul  className={'hidden lg:flex items-stretch gap-0.5 text-white group-hover:text-black-dark grow'}>
                       {menu?.items?.map((menuItem,key)=>
                           <li key={menuItem._id}
                               ...
                               <Visible visible={!!menuItem.children?.length}>
                                   <div className={className('min-w-[200px] absolute -right-1/2 top-full divide-x divide-x-2 rounded-b-md ',{
                                       ...
                                       <Visible visible={!!menuItem?.children?.[activeMenu[1]??0]?.children?.length}>
                                           <ul className={className('flex min-w-[200px] flex-col gap-2 bg-white p-2 divide-y')}>
                                               {menuItem?.children?.[activeMenu[1]??0]?.children?.map((child2,key3)=>
                                                   <li key={child2._id}
                                                       onMouseEnter={(e)=>{
                                                           e.stopPropagation()
                                                       ...
                                                       <Visible visible={!!child2?.children?.length && activeMenu[2]===key3}>
                                                           <ul className={className('absolute top-0 -left-full flex min-w-[200px] flex-col gap-2 bg-white p-2 divide-y')}>
                   ...
                   <div className='flex items-center gap-3 pb-2 lg:gap-6 lg:pb-4'>
                       <Search/>
                       <Link className='flex items-center justify-center' href='#'><Icon name="user-bold" size="w-4 h-4 lg:w-8 lg:h-8" color="fill-white group-hover:fill-red-ehya"/></Link>
                   </div>
               </nav>
           </BaseContainer>
